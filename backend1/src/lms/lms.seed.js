...
const COURSES = [
  {
    title: 'Frontend Masterclass 2024',
    category: 'frontend',
    level: 'intermediate',
    estimatedHours: 12,
  ...
  {
    title: 'Backend Scalability via Node.js',
    category: 'backend',
    level: 'advanced',
    estimatedHours: 8,
    tags: ['backend', 'node', 'architecture'],
    accessTier: 'certified',
    tokenCost: 50,
    isCertified: true,
  },
  {
    title: 'System Design for Real',
...
async function seed() {
  ...
  for (const c of COURSES) {
    ...
    if (!course) {
      ...
      console.log(`Created course: ${c.title} (${c.accessTier}, ${c.tokenCost} tokens)`);
      ...
      console.log(`Updated course tiers: ${c.title} (${c.accessTier}, ${c.tokenCost} tokens)`);
    }
  }
