...
function formatJobResponse(scoredJob) {
  const finalLabel = getMatchLabel(scoredJob.finalScore);
  return {
...
async function runJobMatchingPipeline({ candidate, cleanedResumeText, limit }) {
  ...
  const topJobs = scoredJobs.slice(0, Math.min(5, Math.max(scoredJobs.length, 1)));
  ...
  await Promise.all(
    topJobs.map(async (job) => {
      try {
        ...
        const aiMatch = await getFullAiMatchWithOpenAI(candidateSummaryText, job.rawJob, {
          candidateId: normalizedCandidate.id,
          deterministicScore: job.deterministicScore,
          matchedSkills: job.matchedSkills,
          missingSkills: job.missingSkills,
        });
        const semanticAi = await getSemanticScoreWithOpenAI(normalizedCandidate, job.rawJob);

        const finalMatchedSkills = Array.from(new Set([
          ...job.matchedSkills,
          ...aiMatch.matchedSkills,
          ...semanticAi.matchedSkills,
        ]));

        const finalMissingSkills = Array.from(new Set([
          ...job.missingSkills,
          ...aiMatch.missingSkills,
          ...semanticAi.missingSkills,
        ])).filter((skill) => !finalMatchedSkills.includes(skill));

        const aiFailed = !aiMatch.aiScore && !aiMatch.reasoning && aiMatch.matchedSkills.length === 0 && aiMatch.missingSkills.length === 0;
        const aiScore = aiMatch.aiScore || (semanticAi.score * 10);

        job.breakdown.aiScore = aiScore;
        job.breakdown.semanticBoost = semanticAi.score;
        job.finalScore = Math.min(
          100,
          Math.round(((job.deterministicScore * 0.6) + (aiScore * 0.4)) * 100) / 100
        );
        job.matchedSkills = finalMatchedSkills;
        job.missingSkills = finalMissingSkills;
        job.explanation = aiFailed
          ? `You match this role through ${job.matchedSkills.slice(0, 3).join(', ') || 'strong core alignment'}. Adding ${job.missingSkills.slice(0, 2).join(', ') || 'clearer role-specific evidence'} would improve your fit.`
          : (aiMatch.reasoning || await generateExplanationWithOpenAI(candidateSummaryText, job.rawJob, job));
        job.whyNotMatched = job.finalScore < 50 ? deriveLowMatchReason(job) : null;
        const confidence = computeConfidence(job, aiFailed ? null : aiMatch.aiScore);
        job.confidenceScore = confidence.confidenceScore;
        job.confidenceLevel = confidence.confidenceLevel;
        job.aiEnhanced = !aiFailed;
        aiApplied = aiApplied || !aiFailed;
