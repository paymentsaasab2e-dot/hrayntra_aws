...
function buildSections(candidate) {
  ...
  const candidateFirstName = String(candidate?.firstName || '').trim();
  const resolvedFirstName = candidateFirstName || nameParts.firstName;
  ...
  const sections = [
    {
      ...
      completionRule:
        'First name, email, phone number, gender, date of birth, city, and country are required.',
      schemaFields: [
        'fullName',
        'email',
        'phoneNumber',
      ...
      missingFields: [
        !hasValue(resolvedFirstName) ? 'firstName' : null,
        !hasValue(basicInfo?.email) ? 'email' : null,
        !hasValue(basicInfo?.phoneNumber) ? 'phoneNumber' : null,
        !hasValue(basicInfo?.gender) ? 'gender' : null,
        !hasValue(basicInfo?.dateOfBirth) ? 'dateOfBirth' : null,
        !hasValue(basicInfo?.city) ? 'city' : null,
        !hasValue(basicInfo?.country) ? 'country' : null,
      ],
      isComplete:
        hasValue(resolvedFirstName) &&
        hasValue(basicInfo?.email) &&
        hasValue(basicInfo?.phoneNumber) &&
        hasValue(basicInfo?.gender) &&
        hasValue(basicInfo?.dateOfBirth) &&
        hasValue(basicInfo?.city) &&
        hasValue(basicInfo?.country),
    },
    {
