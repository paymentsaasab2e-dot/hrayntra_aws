...
const DEFAULT_AVAILABLE_TAGS: CandidateTagItem[] = [
  { id: 'tag-senior', label: 'Senior', color: '#2563eb' },
  { id: 'tag-communication', label: 'Strong Communication', color: '#7c3aed' },
  { id: 'tag-referral', label: 'Referral', color: '#059669' },
  { id: 'tag-client-favorite', label: 'Client Favorite', color: '#ea580c' },
  { id: 'tag-urgent', label: 'Urgent', color: '#dc2626' },
  { id: 'tag-remote', label: 'Remote Candidate', color: '#0891b2' },
  { id: 'tag-leadership', label: 'Leadership', color: '#ca8a04' },
  { id: 'tag-technical', label: 'Technical', color: '#4f46e5' },
];
...
function mapBackendCandidate(c: BackendCandidate): Candidate {
  ...
  return {
    id: c.id,
    name: name || c.email,
    avatar: 'https://images.unsplash.com/photo-1655249493799-9cee4fe983bb?auto=format&fit=crop&q=80&w=200',
    ...
    lastActivity: (c.updatedAt || c.createdAt).slice(0, 10),
    hotlist: c.hotlist,
    phone: c.phone || '',
    email: c.email,
    skills: c.skills || [],
...
function CandidatesPageContent() {
  ...
  const [availableDrawerTags, setAvailableDrawerTags] = useState<CandidateTagItem[]>(DEFAULT_AVAILABLE_TAGS);
  ...
  const loadCandidates = useCallback(async () => {
    try {
      ...
      // Build query params from filters and active stage
      ...
      // Map frontend stage IDs to backend stage field values
      if (activeStage !== 'all') {
        ...
        if (backendStage) {
          // Use stage field for filtering (not status)
  ...
  useEffect(() => {
    ...
    async function loadPipelineOptions() {
      try {
        const [jobsRes, usersRes] = await Promise.all([apiGetJobs({ limit: 100 }), apiGetUsers({ limit: 100 })]);
        if (cancelled) return;
  ...
  const handleViewProfile = async (candidate: Candidate) => {
    ...
    setSelectedCandidateProfile({
      ...
      notes: [
        {
          id: `${candidate.id}-note-initial`,
          text: 'Initial candidate review started. Waiting for full profile details.',
          createdAt: new Date().toISOString(),
          recruiter: {
            id: 'current-user',
            name: candidate.owner || 'Recruiter',
            avatar: null,
          },
          tags: ['Screening'],
          isPinned: true,
      ...
      aiScore: {
        ...
        insights: [
          { type: 'strength', text: 'Candidate profile matches several core keywords from the role.' },
          { type: 'gap', text: 'Resume analysis is still loading, so this score is provisional.' },
        ],
      },
      activity: candidate.lastActivity
        ? [
            {
              id: `${candidate.id}-last-activity`,
              type: 'note-added',
              title: 'Latest activity',
              description: 'Candidate profile was recently updated.',
              timestamp: new Date().toISOString(),
              performedBy: {
                name: candidate.owner || 'Recruiter',
              },
              relatedJob: candidate.assignedJobs[0] || null,
            },
          ]
        : [],
    });
  ...
  return (
    <div className="w-full min-h-screen bg-[#f8fafc]">
      ...
      <StageTabs 
        ...
        refreshTrigger={candidates.length} // Refresh stats when candidates list changes
      />
