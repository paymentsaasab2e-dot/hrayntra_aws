...
function resolveRsvpToken(
  paramsToken: unknown,
  queryToken: string | null,
): string {
  const fromQuery = String(queryToken || '').trim();
  if (fromQuery) {
    try {
  ...
  if (fromParams && fromParams.toLowerCase() !== 'interview-rsvp') {
    try {
  ...
  if (parts[0] !== 'interview-rsvp' || parts.length < 2) return '';
  const last = parts[parts.length - 1] || '';
  if (!last || last.toLowerCase() === 'interview-rsvp') return '';
  try {
...
export default function InterviewRsvpClient() {
  const params = useParams();
  const searchParams = useSearchParams();
  // Prefer ?token= (mobile-Gmail safe). Path token kept for older emails.
  const token = resolveRsvpToken(params?.token, searchParams.get('token'));
  ...
  const applyAcceptResult = (data: any) => {
    setInterview(data);
    ...
    setDone({
      kind: 'accept',
      title: 'Interview confirmed',
      message: 'Thanks — your acceptance was sent to the recruiter.',
    });
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Email "Accept" should land on the confirmation screen (no login, no second tap).
        if (action === 'accept' && !autoAcceptStarted.current) {
          autoAcceptStarted.current = true;
          setSubmitting(true);
          try {
            const acceptRes = await fetch(
              `${apiBase}/interviews/public/rsvp/${encodeURIComponent(token)}/accept`,
              { method: 'POST' },
            );
            const acceptPayload = await acceptRes.json().catch(() => null);
            if (!acceptRes.ok || !acceptPayload?.success) {
              throw new Error(acceptPayload?.message || 'Unable to confirm acceptance');
            }
            if (!cancelled) applyAcceptResult(acceptPayload.data);
          } catch (acceptErr: unknown) {
            if (!cancelled) {
              setError(
                acceptErr instanceof Error ? acceptErr.message : 'Unable to confirm acceptance',
              );
            }
          } finally {
  ...
  const submit = async (kind: 'accept' | 'reject' | 'reschedule') => {
    ...
    try {
      const path =
        kind === 'accept' ? 'accept' : kind === 'reject' ? 'reject' : 'reschedule';
      const body =
        kind === 'reject'
          ? { reason: rejectReason }
          : kind === 'reschedule'
      ...
      if (existing) {
        ...
        setDone({
          kind,
          title: kind === 'accept' ? 'Interview confirmed' : 'Response recorded',
          message:
            kind === 'accept'
  ...
  if (loading || (action === 'accept' && submitting && !done && !error)) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 text-slate-600">
        <Loader2 className="mr-2 h-5 w-5 animate-spin" />
        {action === 'accept' ? 'Confirming interview…' : 'Loading interview…'}
      </div>
    );
  }
  ...
  if (done) {
    const isDeclined = done.kind === 'declined' || done.kind === 'reject';
