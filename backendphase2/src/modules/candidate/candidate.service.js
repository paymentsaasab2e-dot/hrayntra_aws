...
/** Prisma scope: non-phase1 OR phase1 with assigned job(s). Avoid nested `some` — hangs Atlas. */
...
function resolveMineJobTake() {
  ...
  if (Number.isFinite(raw) && raw > 0) return Math.min(500, Math.max(50, raw));
  return 200;
}

function resolveMineLinkTake() {
  const raw = Number(process.env.CANDIDATE_MINE_LINK_TAKE);
  if (Number.isFinite(raw) && raw > 0) return Math.min(5000, Math.max(300, raw));
  return 2000;
}

async function prefetchMineLinkedCandidateIds(jobIds) {
  const capped = (Array.isArray(jobIds) ? jobIds : [])
    .map((id) => String(id || '').trim())
    .filter(Boolean)
    .slice(0, resolveMineJobTake());
  if (!capped.length) return [];
  const LINK_TAKE = resolveMineLinkTake();
  const take = LINK_TAKE;
...
async function buildMineCandidatesScope(userId, knownJobIds) {
  ...
  // Phase 1 discovery (no link to this user's jobs) stays on All only.
  // A non-empty assignedJobs list is not enough: foreign/missing job ids still
  // render as Assigned Job "—" and must not leak into My.
  const phase1OnMyJobs = [
    ...(linkedIds.length ? [{ id: { in: linkedIds } }] : []),
    ...(cappedJobIds.length ? [{ assignedJobs: { hasSome: cappedJobIds } }] : []),
  ];
  return {
    AND: [
      { OR: orClause },
      {
        OR: [{ NOT: { source: 'phase1' } }, ...phase1OnMyJobs],
      },
    ],
  };
}
...
/**
 * Stage filter SoT: candidate.stage OR ids prefetched from pipeline / interviews /
 * placements / matches. Never put nested `some` on the Candidate query — with mine
 * scope that hung Atlas for ~300s (empty My Candidates UI).
 */
async function buildStageFilterWhereAsync(stageParam) {
  ...
  try {
    ...
    if (matchingStageIds.length) {
      const pipeRows = await prisma.pipelineEntry
        .findMany({
          where: { stageId: { in: matchingStageIds } },
          select: { candidateId: true },
          take: STAGE_ID_TAKE,
          orderBy: { movedAt: 'desc' },
        })
  ...
  if (key === 'interviewing') {
    const interviewRows = await prisma.interview
      .findMany({
      ...
      .catch(() => []);
    addIds(interviewRows);
  }

  if (key === 'applied') {
    const appRows = await prisma.application
      .findMany({
        select: { candidateId: true },
        take: STAGE_ID_TAKE,
        orderBy: { appliedAt: 'desc' },
  ...
  if (key === 'offered') {
    const rows = await prisma.placement
      .findMany({
        where: { status: { in: OFFERED_PLACEMENT_STATUSES } },
        select: { candidateId: true },
        take: STAGE_ID_TAKE,
        orderBy: { updatedAt: 'desc' },
      })
  ...
  if (key === 'hired') {
    ...
    const rows = await prisma.placement
      .findMany({
        where: { status: { in: HIRED_PLACEMENT_STATUSES } },
        select: { candidateId: true },
        take: STAGE_ID_TAKE,
        orderBy: { updatedAt: 'desc' },
      })
  ...
  if (key === 'rejected') {
    const [placementRows, matchRows] = await Promise.all([
      prisma.placement
        .findMany({
          where: { status: { in: REJECTED_PLACEMENT_STATUSES } },
          select: { candidateId: true },
          take: STAGE_ID_TAKE,
          orderBy: { updatedAt: 'desc' },
        })
  ...
  if (extraIds.size) {
    orParts.push({ id: { in: Array.from(extraIds).slice(0, STAGE_ID_TAKE) } });
  }
...
async function appendCandidateListFilterAndParts(andParts, filters) {
  ...
  if (jobId) {
    // Prefer assignedJobs index; single-job relation lookups are OK (not a huge $in).
    andParts.push({
      OR: [
...
export const candidateService = {
  async getAll(req) {
    ...
    // Company org filter is for All/shared lists. Applying it on top of mine=true can
    // wipe Super Admin My Candidates when personal ownership is empty/untagged.
    if (!mine) {
      const orgScope = await applyOrgCompanyAssigneeWhere(req, {
        assignedToIdField: 'assignedToId',
        createdByField: 'createdById',
    ...
    // Schedule-interview picker: id/name/phone/job only. Skip portal merge,
    // career prefs, and full profile hydration — those made the dropdown stall.
    if (pickerMode) {
      const pickerLimit = Math.min(Math.max(Number.parseInt(String(req.query.limit || '30'), 10) || 30, 1), 40);
      ...
      const jobIds = [
        ...new Set(
          rows
            .map((row) => String(row.assignedJobs?.[0] || '').trim())
            .filter(Boolean),
        ),
      ];
      ...
      const data = rows.map((row) => {
        ...
        const title = jobId ? titleById.get(jobId) : '';
        return {
          id: row.id,
          ...
          assignedJobTitles: title ? [title] : [],
        };
      });
      return formatPaginationResponse(data, 1, pickerLimit, data.length);
    }
  ...
  async getStats(req = {}) {
    ...
    const superAdminScope = buildSuperAdminOwnerScope(req, ['createdById', 'assignedToId']);
    const mineScope = mine ? await buildMineCandidatesScope(userId, myJobIds) : null;
    ...
    const scopedStatsWhere = {
      AND: [
        scopeWhere || {},
        { isDeleted: { not: true } },
        ...(loadCommonPool || mine ? [] : [buildCrmCandidatesListScopeClause()]),
      ],
    };
