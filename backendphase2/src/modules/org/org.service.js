...
export async function resolveViewerOrgScope(req) {
  ...
  // HQ Super Admin / tenant admin see every company. Someone can keep the Admin
  // SystemRole on company 1 — purpose company_head keeps them inside that node.
  const isTenantAdmin =
  ...
  else if (isTenantAdmin && requested) scopeUnitId = requested;

  const isTenantWide = Boolean(isTenantAdmin && !scopeUnitId);
  let unitIds = [];
  let memberIds = [];
  if (scopeUnitId) {
    unitIds = await collectDescendantIds(scopeUnitId);
    memberIds = await userIdsInOrgScope(scopeUnitId);
    if (userId && !memberIds.includes(userId)) memberIds = [userId, ...memberIds];
  }
  ...
  const payload = {
    isTenantAdmin,
    isTenantWide,
    canSwitchCompanies: Boolean(isTenantAdmin),
    hierarchyPurpose: purpose,
    orgUnitId: scopeUnitId,
    homeOrgUnitId: homeId || null,
    unitIds,
    memberIds,
    companies: companies.map((c) => mapUnit(c)),
  };
...
export async function createOrgUnit(req, body) {
  ...
  const attached = await attachExistingPeopleToUnit(unit, body);
...
export async function adoptWorkspaceIntoUnit(req, id) {
  ...
  const attachedCount = await attachExistingPeopleToUnit(unit, { adoptWorkspace: true });
  return { id: String(unit.id), name: unit.name, attachedCount };
}
