  ...
  try {
    switch (actionType) {
      case 'create_lead': {
        ...
        // Push to Undo Stack (Persistence)
        const undoRecord = await undoService.pushToStack({
          actionId,
          userId: user?.id,
          module: 'Leads',
          action: 'CREATE_LEAD',
        ...
        const expiresAtIso = undoRecord?.expiresAt
          ? new Date(undoRecord.expiresAt).toISOString()
        ...
        return { 
          ...
          undoPayload: {
            ...
            expiresInSeconds,
            label: `Undo lead creation (${lead.companyName || 'new lead'})`,
            action: 'DELETE',
            endpoint: '/api/v1/leads',
            method: 'DELETE',
            targetIds: [lead.id],
            uiReverse: {
              ...
              toast: {
                type: 'info',
                message: 'Lead creation was rolled back',
                duration: 3000,
              },
            },
            undoLine: `Lead ${lead.companyName} created.`
          }
        };
      }
      case 'update_lead': {
        const recordId = String(args.record_id || payload.id || '');
        // For updates, we should ideally fetch old data for reversal, but for now we'll focus on Creation undo
        const lead = await leadService.update(recordId, {
          ...payload,
      ...
      case 'create_client': {
        ...
        return { 
          ...
          undoPayload: {
            ...
            module: 'Clients',
            action: 'CREATE_CLIENT',
            undoLine: `Client ${client.companyName} created.`
          }
        };
      }
      case 'update_client': {
      ...
      case 'create_job': {
        ...
        return { 
          ...
          undoPayload: {
            ...
            module: 'Jobs',
            action: 'CREATE_JOB',
            undoLine: `Job ${job.title} created.`
          }
        };
      }
      case 'update_job': {
      ...
      case 'create_task': {
        ...
        return { 
          ...
          undoPayload: {
            ...
            module: 'Tasks',
            action: 'CREATE_TASK',
            undoLine: `Task "${result.title}" created.`
          }
        };
      }
      case 'update_task': {
      ...
      case 'create_placement': {
        ...
        return { 
          ...
          undoPayload: {
            ...
            module: 'Placements',
            action: 'CREATE_PLACEMENT',
            undoLine: `Placement for ${result.candidate?.firstName} created.`
          }
        };
      }
