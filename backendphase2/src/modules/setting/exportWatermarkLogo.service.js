...
const SUBDIR = 'export-watermarks';
...
function sanitizeFilename(name) {
  const base = path.basename(String(name || 'watermark.png').trim());
  return base.replace(/[^a-zA-Z0-9._-]/g, '_') || 'watermark.png';
}
...
/** Store org export watermark logo (S3 or local uploads). */
export async function storeExportWatermarkLogoFile(file, { tenantDbName, userId } = {}) {
  ...
  if (size > MAX_BYTES) {
    const err = new Error('Logo must be under 4 MB');
    err.code = 'VALIDATION';
    throw err;
  }
  ...
  if (isS3Configured()) {
    try {
      ...
      console.warn(
        '[export-watermark-logo] S3 upload failed, using local storage:',
        error?.message || error,
      );
    }
  }
